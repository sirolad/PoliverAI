...
import { twFromTokens, colors, baseFontSizes, fontWeights, textSizes } from '@/styles/styleTokens'
...
export default function FreeReportView({ result }: Props) {
  ...
  return (
    <div data-view="free" id="report-free-view" className={twFromTokens('p-4 rounded h-full min-h-0 overflow-auto w-full', colors.surfaceMuted)}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className={twFromTokens(textSizes.sm, colors.textMuted, 'flex items-center gap-2')}><BarChart className={twFromTokens('h-4 w-4', colors.textMuted)} />{t('policy_analysis.confidence_label', { pct: String(Math.round(Number(result?.confidence ?? 0) * 100)) })}</div>
        </div>
        <div>
          <div className={twFromTokens(textSizes.sm, colors.textMuted)}>{t('policy_analysis.score_label')}</div>
          <div className="flex items-center">
            {Array.from({ length: 5 }).map((_, idx) => (
              <svg key={idx} className={twFromTokens('h-5 w-5', idx < stars ? colors.warning : colors.textLight)} viewBox="0 0 20 20" fill="currentColor" aria-hidden>
      ...
      <div className={twFromTokens('mt-4 whitespace-pre-wrap', textSizes.sm, colors.textPrimary)}>{result?.summary}</div>
      <div className="mt-4">
        <h4 className={twFromTokens(fontWeights.medium, 'flex items-center gap-2')}><AlertTriangle className={twFromTokens('h-4 w-4', colors.danger)} />{t('policy_analysis.top_findings')}</h4>
