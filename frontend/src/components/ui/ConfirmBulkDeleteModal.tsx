    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={getModalContainerClass().replace('max-w-md', 'max-w-lg')}>
        ...
        <div className="p-4">
          {filenames.length === 1 ? (
            <>
              <p className={twFromTokens(textSizes.sm, colors.textSecondary, 'mb-4')}>{t('confirm_bulk_delete_modal.question_single', { filename: filenames[0] })}</p>
              <div className={twFromTokens(baseFontSizes.sm, colors.surfaceMuted, colors.mutedBorder, 'mb-4 rounded p-2 truncate border')}>{filenames[0]}</div>
            </>
          ) : (
            <>
              <p className={twFromTokens(textSizes.sm, colors.textSecondary, 'mb-4')}>{t('confirm_bulk_delete_modal.question_many', { count: filenames.length })}</p>
              <div className={twFromTokens(baseFontSizes.sm, colors.surfaceMuted, colors.mutedBorder, 'max-h-40 overflow-auto mb-4 rounded p-2 border')}>
