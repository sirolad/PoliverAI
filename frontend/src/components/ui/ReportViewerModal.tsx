...
import { twFromTokens, colors, spacing, alignment, fontWeights } from '@/styles/styleTokens'
        ...
        <div className={twFromTokens('flex items-center justify-between', spacing.modalPadding, colors.mutedBorder)}>
          <div className={twFromTokens(alignment.flexRow, alignment.itemsCenter, 'gap-3', 'min-w-0')}>
            ...
            <div className="min-w-0">
              <div className={twFromTokens(fontWeights.semibold, 'truncate max-w-full')}>{title || filename || t('report_viewer.title_report')}</div>
              <MetaLine>{t('report_viewer.meta_line')}</MetaLine>
            </div>
          </div>
          <div className={twFromTokens(alignment.flexRow, alignment.itemsCenter, 'gap-2')}>
