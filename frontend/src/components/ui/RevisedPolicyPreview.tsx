...
export default function RevisedPolicyPreview({ downloadUrl, filename }: Props) {
  ...
  <div className={twFromTokens('mb-2', textSizes.sm, colors.textMuted)}>{t('policy_analysis.revised_policy_preview')}</div>
  ...
  <div className={twFromTokens('mt-2', textSizes.sm, colors.textMutedLight)}>{t('policy_analysis.nothing_here_desc')}</div>
      </div>
    </div>
  )
}
